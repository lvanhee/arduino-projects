...
int effect = 3;             // effect:      is used to differentiate and select one out of the four effects
...
long timeLastAdditionOfNewPosition = millis();
//===================================================================================================================================================
...
void loop(){
  ...
  if(timeLastAdditionOfNewPosition +1000 < millis())
  {
    timeLastAdditionOfNewPosition = millis();
 for(int i = 0 ; i < NUM_LEDS ; i ++)
   {
...
void adjustSpeed(){
  ...
  LEDAccel = constrain(map(readBalance(), -8000, 8000 , maxLEDSpeed, -maxLEDSpeed),-maxLEDSpeed, maxLEDSpeed);
  ...
  //The following lines of code are used to control the direction of the LED animation sequence, and limit the speed of that animation. 
  if (LEDSpeed>0){
    ledPositions[i]++;                                       // Illuminate the LED in the Next position
  ...
  if (LEDSpeed<0){
    ledPositions[i]--;                                       // Illuminate the LED in the Prior position
    if (LEDSpeed<-maxLEDSpeed){
...
void fireStarter(){
      ...
      for(int i = 0 ; i < NUM_LEDS ; i ++)
      {
        int LEDPosition =ledPositions[i];
        if(LEDPosition > 0)
        {
            ledh[LEDPosition] = potVal;   
            bright = random(50, 100);                            // Randomly select a brightness between 50 and 100
...
long lastUpdateAccelero = millis();
int readBalance()
{
  /*if(millis() < 50 + lastUpdateAccelero)
  return;*/
...
void levelSense(){
      ...
      LEDPosition += constrain(map(readBalance(), -8000, 8000, -10, 10), 0 , NUM_LEDS-1);
