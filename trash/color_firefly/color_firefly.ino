...
const long stepDuration = 4000;
const long MAX_LIVENESS = 3600*3/(4000/1000);
//survie:3h
...
void connectWithOtherSelf()
{
  ...
  boundAddToLiveness(MAX_LIVENESS/100);//total refill in 100 iterations of 4 seconds each = 400s
  
  if(isMaxLiveness())
  {
    Serial.println("Merging with my mate!");
    analogWrite(redPin, r*livenessRatio());
    analogWrite(greenPin, g*0.25*livenessRatio());
    analogWrite(bluePin, b*0.25*livenessRatio());
    delay(200);
    analogWrite(redPin, red*livenessRatio());
    analogWrite(greenPin, green*0.25*livenessRatio());
    analogWrite(bluePin, blue*0.25*livenessRatio());
    ...
    if(abs(r-red) < 3 && abs(b - blue) < 3 && abs(g - green) < 3)
    {
      ...
      analogWrite(greenPin, 255*0.25*livenessRatio());
      analogWrite(bluePin, 255*0.25*livenessRatio());
      delay(200);
      analogWrite(redPin, setupRed*livenessRatio());
      analogWrite(greenPin, setupGreen*0.25*livenessRatio());
      analogWrite(bluePin, setupBlue*0.25*livenessRatio());
      delay(200);
      analogWrite(redPin, red*livenessRatio());
      analogWrite(greenPin, green*0.25*livenessRatio());
      analogWrite(bluePin, blue*0.25*livenessRatio());
